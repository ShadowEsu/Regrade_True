...
const PlatformCard: React.FC<PlatformCardProps> = ({
  ...
  const checkRecent = async () => { setImportBusy(true); setImportNote(null); try { const result = await connectorImportService.runAutomatic(meta.platformId); const ignored = [result.ignoredOlderCount ? `${result.ignoredOlderCount} older` : '', result.ignoredNonExamCount ? `${result.ignoredNonExamCount} non-exam` : ''].filter(Boolean).join(' and '); setImportNote(result.imported ? `${result.imported} recent assessment${result.imported === 1 ? '' : 's'} imported.${ignored ? ` ${ignored[0].toUpperCase()}${ignored.slice(1)} record${result.ignoredOlderCount + result.ignoredNonExamCount === 1 ? ' was' : 's were'} left untouched.` : ''}` : `No new recent assessments found.${ignored ? ` ${ignored[0].toUpperCase()}${ignored.slice(1)} record${result.ignoredOlderCount + result.ignoredNonExamCount === 1 ? ' was' : 's were'} left untouched.` : ''}`); } catch (e) { setImportNote(e instanceof Error ? e.message : 'Automatic check failed.'); } finally { setImportBusy(false); } };
  ...
  return (
    <div className="rg3-connector-card" data-platform={meta.platformId}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {canImport && (meta.platformId === 'canvas' || meta.platformId === 'google_classroom') && <button type="button" disabled={importBusy} onClick={() => void checkRecent()} className="rg-btn-ghost text-[13px]">Check recent exams</button>}
      </div>
