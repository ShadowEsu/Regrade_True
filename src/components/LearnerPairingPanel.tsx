...
import { familyService, type FamilyLink, type FamilyPermissions } from '../services/familyService';
...
export default function LearnerPairingPanel() {
  ...
  const updatePermission = async (link: FamilyLink, key: keyof FamilyPermissions) => {
    const current = link.permissions ?? { viewExams: true, viewAiFindings: true, viewAppealDrafts: true, receiveNotifications: true };
    setBusy(true); setError(null);
    try { await familyService.updatePermissions(link.id, { ...current, [key]: !current[key] }); await refresh(); }
    catch (e) { setError(e instanceof Error ? e.message : 'Could not update sharing permissions.'); }
