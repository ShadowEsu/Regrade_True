...
export default function StudyReviewStudio({ exam, onBack }: { exam: Case; onBack: () => void }) {
  ...
  return <div className="space-y-5 pb-8">
    ...
    {!analysis ? <section className="rg-glass-form-card p-6 text-center text-[14px] text-ink-muted">This exam does not have an analysis yet.</section> : <div className="grid gap-5 lg:grid-cols-[minmax(0,1.45fr)_minmax(300px,0.8fr)]">
      ...
      <aside className="rg-glass-form-card flex min-h-[460px] flex-col overflow-hidden"><div className="flex items-center gap-3 border-b border-hairline p-4"><CoachWhale size={44} /><div><p className="rg-meta-k">study coach</p><h2 className="rg-serif text-lg font-semibold text-ink">Ask Mr Whale</h2></div></div><div className="flex-1 space-y-4 p-4"><div className="rounded-2xl bg-primary/[0.05] p-3 text-[13px] leading-relaxed text-ink-muted"><span className="font-semibold text-ink">{active?.questionId || 'This exam'}:</span> {active?.detail || 'Select a marked question to focus the conversation.'}<p className="mt-2 text-[11px] italic">Evidence: {active?.evidence || 'No question evidence extracted.'}</p></div>{answer && <div className="rounded-2xl border border-primary/15 bg-white p-3 text-[13px] leading-relaxed text-ink">{answer}</div>}{asking && <div className="flex items-center gap-2 text-[13px] text-ink-muted"><ICONS.Loader2 className="h-4 w-4 animate-spin text-primary" />Mr Whale is checking the evidence…</div>}</div><form onSubmit={(event) => { event.preventDefault(); void askWhale(); }} className="border-t border-hairline p-3"><label className="sr-only" htmlFor="study-question">Ask Mr Whale about this mark</label><textarea id="study-question" value={input} onChange={(event) => setInput(event.target.value)} rows={3} placeholder="Ask how to improve this answer…" className="w-full resize-none rounded-xl border border-hairline bg-white px-3 py-2.5 text-[13px] text-ink outline-none focus:border-primary" /><button type="submit" disabled={!input.trim() || asking} className="rg-btn-cta mt-2 w-full py-2.5 text-[13px] disabled:opacity-45">Ask Mr Whale</button></form></aside>
    </div>}
  </div>;
}
