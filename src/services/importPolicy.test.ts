...
const item = (id: string, gradedAt: string | null, kind: 'graded_record' | 'file' = 'graded_record'): ConnectorImportItem => ({ externalId: id, platformId: 'canvas', title: `${id} exam`, gradedAt, kind, assessmentType: 'exam' });
...
describe('seven-day automatic import policy', () => {
  ...
  it('leaves recent non-exam assignments in the manual picker', () => expect(automaticImportCandidates([{ ...item('homework', '2026-07-10T10:00:00Z'), title: 'Chapter 3 assignment', assessmentType: 'assignment' }], now)).toHaveLength(0));
  it('accepts a recent assessment title when a provider has no type metadata', () => expect(automaticImportCandidates([{ ...item('quiz', '2026-07-10T10:00:00Z'), title: 'Unit 2 Quiz', assessmentType: 'unknown' }], now)).toHaveLength(1));
});
