...
2. Wire per each feature's WIRING.md. Minimum viable wiring:
   ...
   - Parent mode entry in Profile (K-12 gate); tone learning + draftAppeal into the
     draft step.
   - BottomNav/AppBar/Button/Card swaps where drop-in; Gallery behind preview mode.
...
5. Smoke every view in both themes at 390x844 via preview mode (`npm run dev:preview`;
   port 3000 may be taken — use another port).
