...
Firebase authenticates the user. `AuthGate` synchronizes basic identity to `users/{uid}` and reads `onboardingComplete`. `WelcomeSurvey` updates role, name, institution, preferred platform, notification choice, and completion through `userService`. Preview mode writes only to isolated local storage.
...
`ConnectScreen` uses a registry to describe capabilities and status. Live grants go to the API connection store; preview grants are local and explicitly simulated. Import requests list provider records, apply age/deduplication policy, and create user cases. Today this import path does not guarantee a complete AI analysis job.
