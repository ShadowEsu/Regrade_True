...
src/
  ...
  components/         ~30 components: Layout, BottomNavIcons, AppealFlowShell/Steps,
                      ...
                      CoachComposer, BrandSpinner, PreviewBanner, PlatformRow, Logo, etc.
  lib/                api.ts (fetch wrapper → /api), gemini.ts (client AI path),
                      appealDraft.ts, appealHelpers.ts, imagePrep.ts, pdfPageImages.ts,
                      pdfText.ts, platformUploadGuides.ts, previewFixtures.ts,
                      previewMode.ts (DEMO PATH), profileContext.ts, sanitize.ts,
...
- **Preview mode**: `--mode preview` (`npm run dev:preview`) uses `previewFixtures.ts`;
  this is the demo path that works without API keys.
