...
Supervisor accounts reuse Review navigation but render `SupervisorHub`. This is functional only when the deployed family API is configured and has no synthetic learner fallback.
...
- Development and production share real service paths; missing configuration fails closed.
