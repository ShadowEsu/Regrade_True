...
`VITE_PREVIEW_MODE` is local-only. It must not be enabled in a production build.
...
- Client features must show Needs setup or Coming soon when their public configuration is absent.
