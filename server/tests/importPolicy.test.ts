...
import { automaticImportCandidates, isAutomaticAssessment } from '../src/importPolicy.js';
...
test('automatic imports include only unique graded records from the last seven days', () => {
  const result = automaticImportCandidates([
    { externalId: 'recent', platformId: 'canvas', title: 'Recent math exam', kind: 'graded_record', assessmentType: 'exam', gradedAt: '2026-07-10T12:00:00.000Z' },
    { externalId: 'recent', platformId: 'canvas', title: 'Duplicate exam', kind: 'graded_record', assessmentType: 'exam', gradedAt: '2026-07-11T12:00:00.000Z' },
...
test('manual assignment selection remains available while automatic imports stay exam-only', () => {
  const assignment = { externalId: 'assignment', platformId: 'canvas', title: 'Chapter 4 assignment', kind: 'graded_record' as const, assessmentType: 'assignment' as const, gradedAt: '2026-07-10T12:00:00.000Z' };
  const quiz = { externalId: 'quiz', platformId: 'canvas', title: 'Chapter 4 quiz', kind: 'graded_record' as const, assessmentType: 'quiz' as const, gradedAt: '2026-07-10T12:00:00.000Z' };

  assert.equal(isAutomaticAssessment(assignment), false);
  assert.equal(isAutomaticAssessment(quiz), true);
  assert.deepEqual(automaticImportCandidates([assignment, quiz], now).map((item) => item.externalId), ['quiz']);
});
